...
x1[x1 < 0] #return entries of x1 that correspond to same positsions as where x1 smaller than 0
...
is.na(vec)
sum(is.na(vec))
mean(is.na(vec)) #percentage of values that are na (i.e., average if na is 1 and all else 0)
...
# Generate a matrix
mat <- matrix(c(1:51, rep(NA,4)), ncol=5) #can also specify nrow, but R always fills as columns unless you instruct it otherwise
...
table(gapminder$continent) %>% sort()
...
mod1 <- lm(lifeExp ~ gdpPercap + pop, data = gapminder07)
...
hist(gapminder07$lifeExp,
     main="Life expectancy across states in 2007", 
